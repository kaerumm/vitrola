...
// These are characters that are not allowed to start unquoted strings and they also terminate unquoted strings
export const digits: Readonly<number[]> = Array.from('0123456789')
    .map((c) => c.codePointAt(0)!)
    .sort(numberIncreasing)

export function isDigit(character: Option<string>): boolean {
    if (!character) {
        return false
    }
    return binarySearch(digits, character.codePointAt(0)!) !== null
}
